...
function withDetails(item,{tags,beginner,tempo}){const next={...item,tags};delete next.category;if(beginner)next.beginner=true;else delete next.beginner;if(tempo===undefined)delete next.defaultTempo;else next.defaultTempo=tempo;return next}
  ...
  try{
    ...
    if(req.method==="POST"&&req.url==="/publish"){
      ...
      if(replaceId&&!xml){
        ...
        catalog[index]=withDetails({...catalog[index],title,composer},{tags,beginner,tempo});catalog.sort((a,b)=>a.title.localeCompare(b.title));await writeFile(catalogPath,JSON.stringify(catalog,null,2)+"\n","utf8");
      ...
      if(replaceId){
        ...
        catalog[index]=withDetails({...current,title,composer,fullScorePath,readingPartId:readingPartId||undefined,partCount:parts.length},{tags,beginner,tempo});catalog.sort((a,b)=>a.title.localeCompare(b.title));await writeFile(catalogPath,JSON.stringify(catalog,null,2)+"\n","utf8");
      ...
      try{
        ...
        const item=withDetails({id,title,composer,status:"published",scorePath:`/music/${id}/score.musicxml`,fullScorePath:`/music/${id}/full-score.musicxml`,readingPartId:readingPartId||undefined,partCount:parts.length,viewerPath:`/flute-studio/music/${id}`},{tags,beginner,tempo});
