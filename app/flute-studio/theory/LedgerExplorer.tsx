...
export default function LedgerExplorer({zh,onHear}:{zh:boolean;onHear:(midi:number)=>void}){
  ...
  </svg><label className="ledger-slider"><span>{zh?'更低':'Lower'}</span><input type="range" min="-2" max="6" step="1" value={start} aria-label={zh?'探索更低或更高的音':'Explore lower or higher notes'} onChange={e=>{touched.current=true;setStart(Number(e.target.value));setActive(null)}}/><span>{zh?'更高':'Higher'}</span></label></div>;
}
