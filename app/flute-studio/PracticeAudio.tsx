...
function useAudioEngine(){
  ...
  useEffect(()=>{
    if(!metro)return;
    const audio=getAudio();let beat=0;
    const tick=()=>{const oscillator=audio.createOscillator(),gain=audio.createGain(),strong=accent&&beat%beats===0;oscillator.frequency.value=strong?1320:880;gain.gain.setValueAtTime(strong?.105:.065,audio.currentTime);gain.gain.exponentialRampToValueAtTime(.0001,audio.currentTime+.055);oscillator.connect(gain).connect(audio.destination);oscillator.start();oscillator.stop(audio.currentTime+.06);beat++};
    tick();const timer=window.setInterval(tick,60000/bpm);return()=>window.clearInterval(timer);
